...
class readSerial {
	...
	statusPort(port = "cantitate" | "lungime", errmsg = "") {
		if (this.conexiuneOpen) {//deconectare acidentala nu venita de la client
			this.eroarePort(port == "cantitate" ? eroareCantitate + " " + errmsg : eroareLungime + " " + errmsg, "stop")
		} else {
	...
	readSerialLungime() {
		this.intervalLungime = setInterval(() => {
			if (this.resetConfirm) {
				...
				this.writeEventToLog(this.dateCOM, dataOp + " " + timeOp + " -write - " + buf);
		...
		parser.on('data', (data) => {
			...
			const dataOp = moment().format('YYYY-MM-DD');
			const timeOp = moment().format('HH:mm:ss');
			this.writeEventToLog(this.dateCOM, dataOp + " " + timeOp + " - Lg & Diam - " + row);
