...
async function genRaport(rapObj = {}) {
    try {
        ...
        SELECT
            ...
            DATA as dataorder  
        FROM ${tabCantarire} c
        where DATA BETWEEN '${data_ini}'  and '${data_fin}' order by dataorder asc limit 1000000`;

        if(tabCantarire_ant!="" && an_ini!=an_fin){
            const sql_ant=sql.replace(tabCantarire,tabCantarire_ant);
            sql= "SELECT * from(("+sql_ant +") UNION ALL ("+sql+")) as dd order by dataorder"
        }
